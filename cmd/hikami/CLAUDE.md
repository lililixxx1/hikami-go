...
A: 加 `-tags systray` 编译托盘代码（`tray_windows.go`，依赖 `fyne.io/systray`），`-ldflags='-H windowsgui'` 隐藏控制台窗口。Makefile 提供 4 个 Windows target（命名约定：默认=不内嵌 ffmpeg，`-ffmpeg`=内嵌 ffmpeg）：`build-windows-amd64`（无 ffmpeg，控制台版，`-tags embedded_web`）、`build-windows-amd64-ffmpeg`（内嵌 ffmpeg，控制台版，`-tags embed_ffmpeg,embedded_web`）、`build-windows-desktop`（无 ffmpeg + 托盘 + 无控制台，`-tags 'embedded_web,systray'` + `-H windowsgui`）、`build-windows-desktop-ffmpeg`（内嵌 ffmpeg + 托盘 + 无控制台，`-tags 'embed_ffmpeg,embedded_web,systray'`）。CI release.yml 的 windows 矩阵含 `desktop: true` 变体。托盘模式下 stdout 可能不可写，`initLogFile` 自动改写 `%LOCALAPPDATA%/Hikami-Go/hikami.log`（失败回退 exe 同目录便携模式）。Linux/macOS 走 `tray_other.go` 占位，`initLogFile` 返回 stdout 不写文件。

## 相关文件清单

- `main.go` -- 启动流程、Cookie 加密密钥初始化、依赖组装、`newShutdownCoordinator` 创建 + `runTray` 主线程阻塞、自动触发链（normalize→asr→recap→publish→archive 的 SetOnSuccess 回调；recap→publish 回调按 session.SourceType 拦截回放类自动发布）、archive Handler 创建与 WithBypassFailState 注册、旧版术语表导入、回顾模板 Store 创建。**2026-08-15**:`autoChainCapabilityGate`（M14 实时能力 gate,`runtimeStatusSource` 接口 + `atomic.Pointer` + `attach(server)`）、启动调 `fsutil.RemoveTempCookieFiles`（L4 cookie 残留清扫）、`publisherHandler.SetTaskStore(workerPool.Store())`（M11 发布链幂等 draft_id 持久化装配）。**2026-08-16**:`asrHandler.SetTaskPayloadWriter(workerPool.Store())`（ISSUE-006:ASR submit 后立即把 dashscope_task_id 持久化进 task payload,崩溃恢复重入 await 轮询零重复付费,详见 `internal/asr/CLAUDE.md`）
...
| 日期 | 操作 | 说明 |
|------|------|------|
| 2026-08-16 | BUG 修复 | **ISSUE-006 装配行**(commit `8b81924`,2026-08-17 /init 回填):main.go 在 `asrHandler.SetOnSuccess` 之前加 `asrHandler.SetTaskPayloadWriter(workerPool.Store())` 一行——ASR 提交 DashScope 后立即把 task_id 写进 task payload,进程崩溃恢复时重入 await 轮询而非重新提交付费任务(对齐 M11 `publisherHandler.SetTaskStore` 范式)。完整修复(两阶段接口/哨兵/fail-closed)在 `internal/asr` 包,详见 `internal/asr/CLAUDE.md`;worker.go 仅注释更新。cmd 包测试数不变(4)。 |
...
> **命名变更（2026-07-24）**:产物命名约定从「`-lite`=无 ffmpeg」改为「默认=无 ffmpeg,`-ffmpeg`=内嵌 ffmpeg」,消除「lite=功能阉割」的误解。上文的 `-lite` target 名为当时历史记录,现已重命名(见本文件 FAQ 与 `CLAUDE-detail/development.md`)。 |
