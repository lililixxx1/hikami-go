...
| `(*BuvidStore).Invalidate(cookieHeader)` | 删除指定 cookie 维度的 buvid3/buvid4 缓存条目，下次 `GetBuvids` 重新拉取 finger/spi。用于 -352 风控重试前强制刷新指纹（与 `WBISigner.RefreshKeys` 配合）。**key 必须是传给 GetBuvids 的原始 cookieHeader（injectAntiRisk 注入前的 baseCookie）**，否则删错 key；**nil-safe**（nil 接收者直接返回，不打网络、不动 map） |
...
> **设计要点**：buvid 注入是 -352 风控对抗的**必要但不充分**条件——`getInfoByRoom`/`getRoomInfoOld` 端点还需 **WBI 签名**（`WBISigner.SignURL`）才能完全通过（探针实测：buvid only 仍 -352，buvid + WBI → 200 code=0）。`getDanmuInfo` 端点 buvid + WBI 同样必要。三处调用方（`channel`/`publisher`/`live_record`）统一复用本组件，消除此前两份重复的 buvid 拉取实现。**-352 重试范式**（`live_record/bilibili.go` 的 CheckLive、`live_record/danmaku.go` 的 getDanmuInfo）：首请求 -352 → `RefreshKeys`（刷 WBI）+ `Invalidate(baseCookie)`（失效 buvid 缓存）→ 重试一次，仍 -352 上层走阶梯冷却或降级。
...
- `videoid.go` -- 视频链接解析：`ExtractVideoID`（BV 号优先，非 B 站用归一化 URL 的 sha1 兜底）、`NormalizeSourceURL`（去 fragment/跟踪参数，幂等）、**2026-08-13 PR#1**：`ExtractVideoPart`（p 查询参数显式分 P）+ `ExtractVideoSourceID`（BV+p 追加 `_pNNN` 场次级来源标识，多 P 各分 P 不互相去重）、**2026-08-16**：`SourceIDWithPart`（调用方已持 ID（如 yt-dlp entry.ID 不保证匹配 BV 正则）时锚定该 ID 只追加后缀，与 ExtractVideoSourceID 单一后缀来源保持去重口径一致；**2026-08-19 E2E 修正**:yt-dlp flat-playlist 的 entry.id 常不带 `BV` 前缀,锚定前先从 rawURL 提取 BV 兜底,否则 discover 与 download-by-url 键脱节)
...
- `cover_test.go` -- 封面下载与扩展名推断测试（2 个用例）
- `replay_title_test.go` -- 录播标题清洗测试（2 个用例）
- `http_test.go` -- 测试辅助（mockHTTPDoer/mockHTTPClient，无 Test 函数）
- `buvid_test.go` -- BuvidStore 测试（10 个用例：拉取+缓存、按 cookie 缓存、空 b_3 报错、HTTP 失败、nil-safe、InjectBuvids replace 语义 5 子用例、Invalidate nil-safe/按 key 删除强制重拉/仅影响指定 cookie/缺失 key no-op）

## 变更记录 (Changelog)

| 日期 | 操作 | 说明 |
|------|------|------|
| 2026-08-19 | BUG 修复 | **回归 E2E 抓到 L14 残留**(commit `3fe5b57`):yt-dlp `--flat-playlist` 的 entry.id 不带 `BV` 前缀,`SourceIDWithPart` 锚定 entry.ID 原样保留会生成 `1SW411P7Du_p001` 类无前缀键,与 download-by-url 的 `BV1SW411P7Du_p001` 去重键脱节。修复:videoID 不匹配 BV 正则时从 rawURL 提取 BV 作锚(custom-id+URL-BV 兼容语义保留);`TestSourceIDWithPart` 扩 8 case(videoid_test 函数数不变 10,biliutil 总数 98 不变)。详见 `docs/2026-08-19-回归E2E测试报告-debd7b1到cf6ec70.md`。 |
