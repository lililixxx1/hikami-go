...
提供 B 站 Netscape 格式 Cookie 文件的加载、解析、写入、AES-256-GCM 静态加密、账号池管理、QR Login 扫码登录、WBI URL 签名机制，以及视频 view/playurl/弹幕 XML/seg.so API 客户端。作为共享工具包被 `publisher`、`channel`、`live_record`、`handler`、`download` 等多个模块引用，统一 Cookie 处理、账号解析、API 签名和 B 站 HTTP 请求基础能力。

## 入口与启动

- **入口文件**: `cookie.go`, `cookie_crypto.go`, `cookie_account.go`, `cookie_writer.go`, `login.go`, `wbi.go`, `ua.go`, `video.go`, `playurl.go`, `danmaku.go`, `danmaku_seg.go`
- **核心类型**: `BiliCookie`, `CookieAccountStore`, `QRLoginSessionStore`, `WBISigner`, `VideoClient`, `PlayURLClient`, `DanmakuClient`, `SegDanmaku`, `SegDanmakuClient`
- **测试总数**: 69（按 `grep -c "^func Test" internal/biliutil/*_test.go` 统计；沙箱中 `wbi_test.go` 的 `httptest.NewServer` 会失败，但测试定义仍计入；http_test.go 仅含 helper 无顶级 Test 函数）
