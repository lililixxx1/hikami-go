...
| 方法 | 说明 |
|------|------|
| `NewStore(db, outputRoot ...string)` | 创建 Store 实例。**2026-07-25 改 variadic option 签名**:不传 outputRoot 时字段为空(全项目约 50 处测试调用零改动),仅 `main.go` 生产调用传 `cfg.OutputRoot`、reset 相关测试传 `t.TempDir()`。outputRoot 仅供 `ResetFailedSession` 守卫③.5 校验音频文件存在性 |
...
| `ResetFailedSession(ctx, sessionID)` | **2026-07-21 新增,2026-07-25 加守卫③.5**。把 ASR 失败的 session 从 `failed` 重置回 `media_ready`,作为 UI/API 的「重置失败场次」恢复入口。**6 个守卫**:① session 存在(ErrNotFound)② status=failed(ErrSessionNotFailed)③ local_available=1(ErrLocalFilesRemoved)③.5 **audio.asr.mp3 真实存在**(ErrAudioFileMissing,2026-07-25 新增——local_available 标志位不保证文件未被外部删除/磁盘损坏)④ current_task 类型=asr(ErrResetOnlyForASRFailure)⑤ 无 pending/running task(ErrActiveTaskExists)。守卫⑤(active task)合并进 UPDATE 的 WHERE 子句,SQLite 单连接下原子执行(消除 check-then-act 竞态)。RowsAffected=0 时二次查询区分原因。**不删任何 task**(保留历史供审计),**保留 publish_target/published_at**。5 个哨兵:`ErrSessionNotFailed`/`ErrLocalFilesRemoved`/`ErrResetOnlyForASRFailure`/`ErrActiveTaskExists`/`ErrAudioFileMissing` |
...
- `session_test.go`: 共 50 个测试函数，覆盖：
  ...
  - **`ResetFailedSession`（2026-07-21 新增 9 个 + 2026-07-25 新增 1 个）**：成功重置 / status 非 failed / local_available=0 / 非 ASR 失败（current_task 类型不符）/ 存在 pending/running task / session 不存在 / 空 taskID / task 已删除 / **v7 原子守卫（`TestResetFailedSession_ActiveTaskAtomicGuard` 验证 active task 检查合并进 WHERE 子句的原子性）** / **守卫③.5 音频文件缺失拒绝（`TestResetFailedSession_AudioFileMissing`,2026-07-25 新增）**。6 个越过守卫③的用例通过 `touchResetAudio(t, root)` helper 创建音频文件 + `NewStore(db, t.TempDir())` 注入 outputRoot
...
- `session.go` -- 唯一源文件（含 `ErrAlreadyLive` 哨兵、`isConstraintViolation` helper、`ResetFailedSession` 方法 + 5 个 reset 错误哨兵）
- `session_test.go` -- 单元测试（50 个测试函数）

## 变更记录 (Changelog)

| 日期 | 操作 | 说明 |
|------|------|------|
| 2026-07-25 | BUG 修复 | **ResetFailedSession 加守卫③.5 + variadic NewStore**(branch `fix/media-ready-consistency-2026-07-25`,qoder 计划审核 Ready with fixes + 代码审核 Ready)。**触发**:用户反馈界面上 64 个「音频已就绪」实际仅 1 个本地真有音频文件(qoder 独立审核确认是 2026-07-16 批量脏数据,normalize convertAtomic 空输出已在本轮 Phase 1 堵住根因)。**本 Phase 2 加固 reset 路径**:`local_available` 标志位(守卫③)不保证文件未被外部删除/磁盘损坏,故新增守卫③.5——reset 前 `os.Stat(<outputRoot>/<channel>/<slug>/asr/audio.asr.mp3)`,缺失返回新哨兵 `ErrAudioFileMissing`(handler writeError 映射 409)。**variadic NewStore(db, outputRoot...)**:outputRoot 未注入时保守放行(向后兼容 50 处测试调用),仅 `main.go` 传 `cfg.OutputRoot`、6 个越过守卫③的 reset 测试传 `t.TempDir()` + `touchResetAudio` helper 创建音频文件。新增 `TestResetFailedSession_AudioFileMissing`,session_test.go 49→50。**同时修正文档**:07-21 changelog 里把 5 个哨兵误写成 4 个且名字过时(`ErrResettableConditionFailed`/`ErrInvalidResetState` 实际不存在,真名见对外接口段),本轮一并订正。见 `plans/plan-media-ready-consistency-2026-07-25.md` Phase 2。 |
| 2026-07-21 | 功能/BUG 修复 | **新增 `ResetFailedSession` 方法**(branch `fix/bug-fix-2026-07-20`,commit `61f3989` v6 + `add3b51` v7)。**触发**:ASR 任务失败后 session 进入 `failed` 状态,重提 ASR 返回 `409 status must be media_ready`,**无任何 UI/API 恢复入口**——只能直接改 DB + 重启服务。**v6**:5 守卫(session 存在 / status=failed / local_available=1 / current_task 类型=asr / 无 pending/running task)+ UPDATE 的 WHERE status='failed' 二次校验 + RowsAffected 检查;不删 task(保留审计);保留 publish_target/published_at。**v7 修订**(r20 HIGH):把 active task 检查从独立 SELECT 合并到 UPDATE 的 WHERE 子句(`NOT EXISTS pending/running task`),SQLite 单连接下原子执行,消除 check-then-act 竞态;RowsAffected=0 时二次查询区分 ErrNotFound/ErrSessionNotFailed/ErrActiveTaskExists。哨兵:`ErrSessionNotFailed`/`ErrLocalFilesRemoved`/`ErrResetOnlyForASRFailure`/`ErrActiveTaskExists`(2026-07-25 加 `ErrAudioFileMissing` 共 5 个)。新增 9 个测试(8 个守卫路径 + 1 个 v7 原子守卫),session_test.go 40→49。 |
