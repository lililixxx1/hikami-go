...
- `manager_anomaly9_test.go`: 8 个测试用例，覆盖异常 #9 的 -352 冷却机制：
...
- `types.go` -- 类型定义、常量、接口、`ErrRiskControl352` 哨兵
- `manager.go` -- Manager 实现、任务执行、Cookie 查找、流选择逻辑、自动录制逻辑、-352 频道级阶梯冷却（`cooldown352Until`/`cooldownStep`）、`ensureStartAllowed`/`startWithInfo`、重连按错误类型分支、`Adopt`/`globLatestAudio`/`updateCurrentOutputPath`
- `bilibili.go` -- BiliClient 实现、B 站 API 交互、流优先级算法、CheckLive -352 单次重试（RefreshKeys + Invalidate）、`signerForCookie`/`injectAntiRisk`
- `danmaku.go` -- 弹幕 WebSocket 协议、BilibiliDanmakuRecorder、消息解包、-352 三级回退
- `ffmpeg.go` -- FFmpegRecorder 实现、HTTP pipe 机制、FLV 自动检测、优雅停止
- `bilibili_test.go` -- B 站客户端测试（含 4 个 -352 重试/哨兵测试）
- `ffmpeg_test.go` -- FFmpeg 录制器测试
- `manager_test.go` -- Manager 集成测试（含重连按错误类型分支、preflight、Adopt）
- `manager_anomaly9_test.go` -- 异常 #9 的 -352 频道级冷却测试（8 个用例）
