...
- `manager_test.go`: 55 个测试用例，覆盖：
  ...
  - **重连预算耗尽统一收口**(2026-08-08):CDN 耗尽+有音频→成功保留(`TestHandleTaskCDNBudgetExhaustedWithAudioFinishesSuccess`)、selectStream 耗尽+有音频→成功保留(`TestHandleTaskSelectStreamBudgetExhaustedWithAudioFinishesSuccess`),钉死 `finishWithAudioIfAny` helper 的四条对称守卫
  - **重连分段统一编号**(2026-08-15 M5):`TestHandleTaskMixedCDNAndReconnectSegmentsUnique`——CDN 重试与 afterRecord 重连混走时分段文件名仍唯一(part.1/2/3 精确断言,旧 dual counter 在区间重叠时互相覆盖)
  - **取消收尾**(2026-08-15 H3):`TestHandleTaskTaskCancelledAfterAudioPreservesAndFinalizes`——任务级取消后已录音频仍保留并 finalize 送 normalize,session 不再卡 recording
...
- `manager.go` -- Manager 实现、任务执行、Cookie 查找、流选择逻辑、自动录制逻辑、**风控频道级阶梯冷却**（`cooldownRiskUntil`/`applyCooldownRiskControl`/`resetCooldownRiskControl`/`cooldownStep`,原 `*352` 命名 v3 改为泛化 `RiskControl` 覆盖 -352 + HTTP 412/403/429）、`isRiskControlError` helper、`ensureStartAllowed`/`startWithInfo`、重连按错误类型分支（含 `afterRecordProbeFailReconnect` + `probeErrorBudget` 异常 #10）、**重连预算耗尽统一收口 `finishWithAudioIfAny`**（CDN/selectFail/afterRecord/probe-error 四分支对称：有已录音频→成功收尾送 normalize，无→返回 carryErr，2026-08-08）、**重连分段统一编号 `segmentSeq`**（2026-08-15 M5:单一计数器替代 CDN/afterRecord 双计数器,混走时 part 文件名不重叠互覆盖）、**取消收尾 `context.WithoutCancel` 派生 ctx**（2026-08-15 H3:任务级取消后收尾/合并/入队仍能跑完,session 不卡 recording）、`Adopt`/`globLatestAudio`/`updateCurrentOutputPath`、**健康检测**（`checkOneChannelHealth` 分发 + `healthStats` 聚合 map + `applyHealthStat`/`peekAbortReason`/`markAbort`/`handleSelectStreamRiskControl`，异常 #11）
...
| 日期 | 操作 | 说明 |
|------|------|------|
| 2026-08-15 | BUG 修复 | **H3 取消收尾 WithoutCancel + M5 重连分段统一编号**(commit `fe442c5`/`96c4e17`,审核批次 P0/P1)。**H3 根因**:重连循环外的取消收尾特例(`:1040-1054`)用任务级 ctx 跑 finalize/合并/入队——ctx 已被取消,`enqueueNormalize`/DB 操作全部立即失败,session 永卡 `recording`(需人工改 DB)。**修复**:收尾路径改 `context.WithoutCancel(taskCtx)` 派生(保留 values 剥离取消链),取消后已录音频仍能合并 + 送 normalize(+1 测试 `TestHandleTaskTaskCancelledAfterAudioPreservesAndFinalizes`)。**M5 根因**:分段命名用 `cdnAttempt` 与 `attemptsUsed` 双计数器,CDN 重试与 afterRecord 重连混走时两者区间重叠 → `audio.part.N.m4a` 同名互相覆盖(分段静默丢失)。**修复**:统一 `segmentSeq` 单计数器,三处产段点共用(+1 测试 `TestHandleTaskMixedCDNAndReconnectSegmentsUnique`,part.1/2/3 精确断言)。manager_test 53→55,live_record 91→**93**。**行为变化**(计划 §7,AGENTS.md 2026-08-16 记录):任务取消从「可能卡死」变为「音频保留 + 收尾入队」。 |
| 2026-08-03 | BUG 修复 | **重连耗尽补「已录音频保留」守卫,修复下播瞬间重连误判致整场失败**(commit `5504d09`,manager.go +22/-3、manager_test.go 改 2 测试,systematic-debugging 四阶段)。**触发**:8-2 灰泽满 Hazel 一场 2h21m 直播 `audio.part.3.m4a`(221MB/8492s 完整)录到了但流水线一次没跑,session 在 live_record 阶段被判 `failed`。**根因**:`manager.go` 重连循环的 `afterRecordReconnect` 分支(明确仍开播的重连)额度耗尽时**无条件 `err = carryErr; break`,无 `hasRecordedAudio` 守卫**;与下方 `afterRecordProbeFailReconnect` 分支(后者耗尽时检查 `hasRecordedAudio`,有音频就 `err=nil` 保留送 normalize)形成不对称。8-2 时序:ffmpeg 录到 2h21m 收上游 EOF 干净退出 → `decideAfterRecord` 探测仍 `live:true`(B 站下播滞后)→ 走 `afterRecordReconnect` → 此后重连全 404(真下播 CDN 撤流)→ 预算耗尽命中无守卫分支 → 整场判 failed,221MB 音频白录。**修复**(对称补守卫,零回归):`afterRecordReconnect` 耗尽分支补 `hasRecordedAudio` 判定——有已录分段→`err=nil` 走成功收尾(`finalizeAudioSegments` 合并 + `enqueueNormalize`),与 probe-error 分支对称;无分段→`err=carryErr` 失败(不送 normalize,避免空音频污染回顾)。**测试改动**(钉死行为):① `TestHandleTaskCleanEOFLiveReconnectExhaustedReturnsSentinel` 改名 `...PreservesAudio`,断言从「期望 errStreamEndedWhileLive 哨兵」翻转成「期望 nil + audio.m4a 合并两段保留」;② `TestHandleTaskCDNRetryThenCleanEOFGoesThroughDecideAfterRecord` 断言同步翻转。两测试核心价值不变,只翻转了耗尽收尾的结论。(2026-08-08 该守卫进一步泛化为 `finishWithAudioIfAny` 统一收口,见下条。详见 `AGENTS.md` 2026-08-03 changelog。) |
| 2026-08-08 | 修复 | **重连预算耗尽统一收口 — CDN/selectFail 补守卫**(codex 四阶段:r12 根因复核 ANALYSIS_PARTIAL → r13/r14 NEEDS_FIX → r15 APPROVED)。**触发**:2026-08-07 灰泽满那场(`bili_1298779265_live_20260807_225255`)live_record 判 failed,但 `audio.part.4.m4a`(36.7MB/1535s 完整)白录。**根因**:8-3 commit `5504d09` 给"重连预算耗尽"补 `hasRecordedAudio` 守卫时只覆盖 `switch decision` 里两条(`afterRecordReconnect`/`afterRecordProbeFailReconnect`),**漏了循环顶部的 CDN 预算耗尽分支**(裸 `break reconnect` 无守卫)和 **selectFailedPending 预算耗尽分支**(同样无守卫)。8-7 那场 part.4 录满后再次断流,后续重连全 404,最后一轮 CDN 404 命中循环顶部 CDN 分支抢先 `break`,带着 404 出循环 → `return err` 丢弃音频。**修复**(codex 更根本方案):① 新增 helper `finishWithAudioIfAny(scenario, channelID, roomID, audioSegments, carryErr)`——有已录音频→返回 nil(成功收尾),无音频→返回 carryErr(失败);② CDN + selectFail 两条补对称守卫;③ 现有 afterRecord/afterRecordProbeFail 两处内联逻辑改调 helper(DRY);④ 4 处均带 scenario 日志(cdn/select-stream/after-record/probe-error)+ channel_id/room_id 恢复生产排查上下文。**不碰风控中止/afterRecordFinishError/ctx.Canceled**(codex 警告:无差别放行会掩盖风控和真实失败)。**测试** +2:`TestHandleTaskCDNBudgetExhaustedWithAudioFinishesSuccess`(`recordOnceThenCDNFailRecorder` 首段成功+后续 404,断言 err==nil + 音频保留 + recorder==7 次)+ `TestHandleTaskSelectStreamBudgetExhaustedWithAudioFinishesSuccess`(同理 selectStream 耗尽)。负向基线已有测试钉死不变。manager_test.go 51→53,live_record 总测试 89→91。**验证**:`go test` 全过(153s,零回归)、vet/gofmt 干净、embedded_web 编译成功。**回归**:零(默认路径不经耗尽分支)。 |
