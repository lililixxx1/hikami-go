...
| `Config.EnsureDirs()` | 创建 `output_root`、`logs.dir`、数据库目录 |
...
- `config_test.go`: 19 个测试用例，覆盖：
  ...
  - 下载后端 helper: TestDownloaderConfigHelpers
  - **Effective\* 默认值**：TestRecapAIEffectiveDefaults、TestDashScopeEffectiveAPIKeyEnv、TestASRS3EffectiveAccessKeyEnv（字段为空时返回约定默认值）
...
- `config.go` -- 唯一源文件
- `config_test.go` -- 配置加载与验证测试（19 个用例）
