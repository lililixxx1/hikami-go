...
// ---------------------------------------------------------------------------
// Test 12: TestApplyRevertPublish — published -> uploaded，清空 publish_target（保留 published_at）
// ---------------------------------------------------------------------------

func TestApplyRevertPublish(t *testing.T) {
	database := setupDB(t)
	insertChannel(t, database)
	insertSession(t, database, "sess_revert", "published")
	// 预置 publish_target 与 published_at（模拟已发布专栏）
	_, err := database.Exec(
		`UPDATE sessions SET publish_target = ?, published_at = datetime('now') WHERE id = ?`,
		`{"dyn_id":"dyn_old"}`, "sess_revert",
	)
	if err != nil {
		t.Fatalf("preset publish_target: %v", err)
	}

	store := NewStore(database)
	ctx := context.Background()

	next, err := store.ApplyRevertPublish(ctx, "sess_revert", "")
	if err != nil {
		t.Fatalf("ApplyRevertPublish: %v", err)
	}
	if next != StatusUploaded {
		t.Fatalf("result = %s, want uploaded", next)
	}

	var status, publishedAt string
	var publishTarget sql.NullString
	err = database.QueryRowContext(ctx,
		"SELECT status, published_at, publish_target FROM sessions WHERE id = 'sess_revert'",
	).Scan(&status, &publishedAt, &publishTarget)
	if err != nil {
		t.Fatalf("query: %v", err)
	}
	if status != "uploaded" {
		t.Fatalf("status = %s, want uploaded", status)
	}
	if publishedAt == "" {
		t.Fatal("published_at 应保留历史值，但为空")
	}
	if publishTarget.Valid && publishTarget.String != "" {
		t.Fatalf("publish_target 应被清空，got %q", publishTarget.String)
	}
}
