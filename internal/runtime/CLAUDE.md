...
| 字段 | 说明 |
|------|------|
| `Version` | 版本标识（`embedded-minimal-7.x-vad2`，标识嵌入的裁剪版 ffmpeg，2026-07-27 起含 VAD 用的 silencedetect/atrim/concat filter + pcm_s16le encoder）；改 Version 会让旧用户升级后重新解包，避免用到旧缓存 |
| `ArchiveURL` | 归档下载 URL |
