...
- `discover_test.go`: 34 个测试函数（2026-07-19 v3: 20→32;2026-07-25: 32→34）,覆盖 DiscoverAll（建任务/跳过已存在/跳过 live_only）、discover_limit（达限/0=不限）、**PreviewAll**（标注 Exists、尊重 discover_limit、达限时在已存在项后 break）、**Execute**（不重跑 yt-dlp、幂等去重）、title_prefix 匹配（原始标题 before CleanReplayTitle / 空 resolver 结果 guard / limit 前置检查）、**cookie 解析 v3 新增**(URL 模式 + 频道模式 + 加密场景 + cleanup,共 13 用例)、**2026-07-25 account_id 选择**(删 2 旧 CookieFile 测试 + 加 4 新 account_id 测试,含 NotFound_FallsBackToDefault fixture 必须设默认账号验证 fall through)。
- `title_concurrency_test.go`: 2 个测试函数（2026-07-29 新增）,覆盖 `previewFromEntries` 有界并发（`TestPreviewFromEntries_ConcurrentTitleResolution`:慢 resolver + CAS 记录 maxInflight,断言耗时显著小于串行 + 最大并发在 1..previewTitleConcurrency 之间）+ 顺序保持（`TestPreviewFromEntries_PreservesOrder`:并发写回后 results 顺序与输入一致）。
- discover 包现 37 个测试函数（2026-08-16 +1:`TestDiscoverChannelMultiPartSourceIDsDistinct`,多 P 合集各分 P SourceID 含 `_pNNN` 后缀互不去重）。

## 相关文件清单

- `discover.go` -- 核心源文件（DiscoverAll / PreviewAll / Execute / DiscoverChannel / PreviewChannel / Preview + `annotateExists` 辅助）。**`previewFromEntries` 2026-07-29 改有界并发**(`previewTitleConcurrency=5` semaphore):title_prefix 过滤在并发前(用原始标题),`resolveTitle` 对空标题视频走 5 路并发,结果按原始 index 写回保序,semaphore 获取用 select 响应 ctx 取消。配套 download 侧 `Handler.viewClient` 长生命周期修复,把发现回放预览 38 条回放从 ~29s 压到 ~3-5s。**2026-08-13 PR#1**:建场次/预览结果用 `biliutil.ReplayDateFromTitle` 从标题提取日期填 `StartedAt`(回放场次有真实开始时间)。**2026-08-16 L14**:`DiscoverChannel` 与预览 Result 的 SourceID 用 `biliutil.SourceIDWithPart(entry.ID, entryURL(entry))` 锚定 entry.ID 追加 `_pNNN` 分 P 后缀,同 BV 多 P 合集各分 P 不再互相去重吞掉(discover_limit_reached/title_prefix_mismatch 日志保留裸 entry.ID——sourceID 尚未计算,见 4ae6604 审核确认)。
- `cookie.go` -- **2026-07-19 v3 新增**:发现阶段 cookie 解析(`resolveURLCookie` / `resolveChannelCookie` / `writePreviewTempCookie`)
- `title_concurrency_test.go` -- `previewFromEntries` 有界并发 + 顺序保持测试（2026-07-29 新增,2 用例）

## 变更记录 (Changelog)

| 日期 | 操作 | 说明 |
|------|------|------|
| 2026-08-16 | BUG 修复 | **L14:多 P 合集 SourceID 追加分 P 后缀**(commit `e3f0e27` + 日志对齐 `4ae6604`,审核批次 P2)。**根因**:discover 建场次用裸 `entry.ID` 作 SourceID,同一 BV 的多 P 合集各分 P 互相去重,勾选多个分 P 只建一个场次(其余被 `already_exists` 吞掉)。**修复**:`DiscoverChannel` 与预览 Result 改用 `biliutil.SourceIDWithPart(entry.ID, entryURL(entry))`(锚定 entry.ID 追加 `_pNNN`,计划原方案 ExtractVideoSourceID 的 sha1 兜底会改变非 BV 去重口径,弃用);预览与执行两路径口径一致。discover_limit_reached/title_prefix_mismatch 日志保留裸 entry.ID(此时 sourceID 尚未计算,审核确认有意保留)。测试 +1(`TestDiscoverChannelMultiPartSourceIDsDistinct`),discover 36→**37**。**一次性影响**:修复前已被吞掉的历史分 P 不会被补建,仅对之后的发现生效。 |
| 2026-08-13 | 功能 | **批量回放可靠性 PR #1**(commit `c9a3e51`):`DiscoverChannel`/预览路径用 `biliutil.ReplayDateFromTitle` 从录播标题(清洗前后两级尝试)提取日期填 `StartedAt`,回放场次不再缺开始时间。无新增测试(日期提取测试在 biliutil replay_date_test)。 |
| 2026-07-29 | 性能修复 | **`previewFromEntries` 标题解析改有界并发**(branch `fix/discover-title-perf-2026-07-29`,qoderclicn 计划审核 Ready with fixes + 代码审核 Ready,reasoning high)。**触发**:用户粘贴合集 URL 发现回放「发现不了」,yt-dlp 列出 38 条但请求耗 29s 被前端 30s 超时砍掉。**根因**:`previewFromEntries` 串行 `resolveTitle`,yt-dlp `--flat-playlist` 对合集页几乎全返回空标题 → 几乎每条回源 view API,叠加 download 侧 buvid/WBI 缓存击穿,38×3 串行请求超时。**改动**:`previewFromEntries` 改有界并发(`previewTitleConcurrency=5` 常量,semaphore + WaitGroup):title_prefix 过滤在并发前(用原始标题),`resolveTitle` 走 5 路并发,结果按原始 index 写回预分配 slice 保序,semaphore 获取用 select 响应 ctx 取消(qoderclicn Minor#1)。`DiscoverChannel`(建场次+入队,有副作用 + createdCount 计数)保持串行不变。**测试**:discover 34→36(+2:`TestPreviewFromEntries_ConcurrentTitleResolution` 慢 resolver + CAS maxInflight 断言并发度与耗时 + `TestPreviewFromEntries_PreservesOrder`)。**回归**:零(纯预览路径无副作用,有界并发 + 按序写回语义等价)。配合 download 侧 viewClient 缓存修复 + 前端 90s 超时兜底。详见 `plans/plan-discover-title-perf-2026-07-29.md`。 |
| 2026-07-25 | 功能 | **发现回放 Cookie 选择改为选已登录 B 站账号**(commit `860a581`,branch `fix/discover-account-select-2026-07-25`,qoder 计划审核 Ready with fixes + 代码审核待调)。**触发**:用户反馈「发现回放」抽屉的「Cookie 文件路径」输入框要手填路径,体验差;且账号池 cookie 是 `HIKAMI_V1` AES-GCM 加密的,用户手填加密路径会让 yt-dlp 静默失败。**改动**(API 契约破坏性更改,仅前端单一调用方,无第三方依赖):① `POST /api/sessions/discover/preview-by-url` body 从 `{cookie_file?: string}` 改为 `{account_id?: integer}`;② `resolveURLCookie` 插入「显式 account_id」分支,复用 `ResolveCookie` 三级链(case 1 命中 / fallthrough 默认),与频道模式 `resolveChannelCookie` 行为一致,**消除两份重复逻辑**(07-19 v3 拆出的 URL/频道两 helper 现统一走 ResolveCookie);③ qoder I-2:加 `GetByID` 预检查 + WARN(ResolveCookie case 1 失败静默 fallthrough 到默认账号时让 fallthrough 可观测);④ qoder M-1:用 `nullInt64FromPtr` helper 替代内联 `sql.NullInt64{...}`。**测试**:discover 32→34(删 2 旧 CookieFile 测试 + 加 4 新 account_id 测试,含 qoder I-1 修订的 `NotFound_FallsBackToDefault` fixture 必须设默认账号验证 fall through 而非空串)。**回归**:零(account_id 留空时行为与 07-19 v3 默认账号路径完全一致)。 |
