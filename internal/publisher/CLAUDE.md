...
- `bilibili_opus_test.go`: 5 个测试函数（含 10 个 t.Run 子测试），覆盖 mapBiliError 错误映射（保留原始 message）、SaveDraft/PublishOpus/DeleteDraft/RemoveOpus/UploadCover HTTP 交互。
...
- `publisher_test.go`: 29 个顶级测试函数（含 t.Run 子测试），覆盖：
  ...
  - publishRecap: 共用发布逻辑（HandleTask/EditOpus 共享）
  ...
  - EditOpus: 删旧+用最新 recap 重发（状态保持 published，「先发新再删旧」安全序）
