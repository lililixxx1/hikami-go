...
- 主播 `cookie_file`: Netscape 格式 cookie 文件路径(legacy fallback)
- Cookie Account: `resolvePublishCookie` 调 `ResolveCookie(ctx, null, nullInt64FromPtr(ch.PublishAccountID), "publish", ch.CookieFile)` 三级链:
  1. **channel.publish_account_id**(2026-07-20 新增,让 level 1 真正生效;此前永远传空 `sql.NullInt64{}` 导致跳过)
  2. 全局默认发布账号(`bili_cookie_accounts.is_default_publish=1`)
  3. legacy `channel.cookie_file` 兜底
...
- `c.buvids.GetBuvids(ctx, cookieHeader)`：调用共享的 `biliutil.BuvidStore` 获取 `buvid3` 和 `buvid4`，按 `cookieHeader` 为 key 缓存 24 小时（与 `live_record`/`channel` 共用同一套实现）。
- `biliutil.InjectBuvids(cookieHeader, buvid3, buvid4)`：把 buvid 注入 Cookie 请求头，采用 **replace 语义**（剔除旧 buvid3=/buvid4= 段再追加新值，避免重复同名 cookie）。
- 指纹获取失败时仅打印 warn 日志，不阻断主流程（继续使用无 buvid 的 Cookie）。
- 2026-07-05 重构：本地 `getBuvids`/`injectBuvids`/`cachedBuvid`/`buvidCache`/`buvidCacheMu` 已下沉到 `biliutil.BuvidStore`，消除与 `live_record/danmaku.go` 的重复实现。
...
- `publisher_test.go`: 34 个顶级测试函数（含 t.Run 子测试），覆盖：
