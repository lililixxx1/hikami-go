...
在 SQLite 数据库中存储和管理 API Key（如 DashScope、AI 回顾等），支持值掩码显示、加载到环境变量、已知 Key 校验、全量清除。作为 Web 设置页面的后端，让用户通过 Web 界面配置 API Key，无需手动编辑环境变量。全量清除用于配置导入的 overwrite 策略。
...
- `secrets.go` -- Store 实现、掩码、校验、视图构建、Clear 全量清除
- `secrets_test.go` -- 单元测试（8 个用例）
