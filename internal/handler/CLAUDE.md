...
- `server_test.go` -- 集成测试（59 个测试函数，表驱动子测试展开后运行时 98 用例；含 fake 依赖实现 + 测试辅助函数、stats/dashboard 死锁回归测试）。handler 包函数口径总数 75（server_test 59 + config_export_test 11 + auth_test 5）。
