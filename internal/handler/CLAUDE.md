...
| **reset 端点哨兵**:`ErrSessionNotFailed` / `ErrLocalFilesRemoved` / `ErrResetOnlyForASRFailure` / `ErrActiveTaskExists` / `ErrAudioFileMissing`(2026-07-25 新增) | 409 |
...
- `server.go` -- 路由注册、API 处理函数、错误映射、统计仪表板、术语表端点（CRUD+导入导出+批量操作）、回顾内容端点、Cookie Account（含 ValidateCookiePath 校验）、全局发布配置、WebDAV/DashScope/ASR-S3/archive/tools/MCP/VAD/Replay 配置端点、**2026-07-27 新增 `GET/PUT /api/config/vad` 端点**(ASR 前置 VAD 静音裁剪配置,DetectionMode 不暴露因 VADProcessor 固定 peak;保存后无需 refreshRuntimeStatus 因 VAD 无 runtime 探测;inline 调 `VADConfig.Validate()` 拒非法值避免存入非法值下次启动 fatal)、**2026-07-30 新增 `GET/PUT /api/config/replay` 端点**(回放类全局自动开关,无密钥无 Validate,presence-aware 覆盖同 VAD 范式;**同日删除 `POST /api/sessions/:sid/recap-partial` + `recap-with-range` 端点**(自定义时间段回顾功能下线,`generateRecapPartial`/`generateRecapWithRange` handler 删除))、回顾模板导入导出、QR 码登录端点、onboarding 引导端点、通知测试端点、诊断报告、WebSocket Origin 校验、配置导出/导入、archiveSession 端点、recap/regenerate 重新生成端点、两步式发现端点（discoverPreviewAll/discoverExecute）、**2026-07-21 新增 `POST /api/sessions/:sid/reset` 端点**（ASR 失败 reset 入口，调 `session.ResetFailedSession`，`writeSessionDetail` helper 抽取复用，writeError reset 哨兵 case 全部 409 Conflict;**2026-07-25 新增 `session.ErrAudioFileMissing`→409 case** 配合守卫③.5）、运行时状态代际校验（`configGen`/`runtimeMu`/`configSnapshot`/`bumpConfigGen`/`refreshRuntimeStatus`/`currentRuntimeStatus`）。**2026-08-13 PR#1**:`jsonBindErrorMessage`(JSON 类型不匹配返回字段级提示,`json.UnmarshalTypeError`→`invalid json type: expected number, got string`)。**2026-08-15 审核批次**:H1 `/ws` WebSocket 补 token 鉴权(query/header 均可,未配置 token 时保持匿名兼容;写超时防护防慢客户端卡住广播);H2 onboarding `dismissed` 判断修正(secrets.Get 不存在返回空串致 needed 恒 false,引导条永不消失);P0 配置导入 secrets key 白名单(任意 key 如 HTTP_PROXY 不能再经 os.Setenv 注入进程环境);P0 MCP GET Authorization 脱敏回显 + PUT 哨兵还原(`__REDACTED__` 哨兵回传不覆盖已存密钥);M12 copy-config 删除源值反向污染(真正复制源主播的值)+ 删账号 409 明细用 `strings.Cut` 取哨兵后缀引用数;L2 importSession 加 2GiB 上限(ContentLength 预检 + MaxBytesReader + `*http.MaxBytesError`→413);删除死代码 `applySecretEnvChange`(51 行)
- `auth.go` -- admin token 认证中间件（X-Admin-Token / Bearer、`subtle.ConstantTimeCompare`）
- `config_export.go` -- 全量配置导出/导入处理器（ConfigExportBundle 8 段指针化含 MCP+VAD + WebDAVExportSection/ASRS3ExportSection/**MCPExportSection** 密钥剔除投影 DTO、handleExportConfig、handleImportConfig 两阶段事务化、validateImportedSections 持久化前校验、channelToUpsertInput；**2026-07-23 新增 MCP 段**:mcpToExport/mcpFromExport/mcpServerSecretKey 三个 helper,MCP 密钥(Builtin.BraveAPIKey/TavilyAPIKey + Servers Headers Authorization)随 Secrets 走,server 鉴权头用「下标+名」双键 `MCP_SERVER_{idx}_{NAME}_AUTHORIZATION` 防归一化碰撞；**2026-07-27 新增 VAD 段**:`bundle.VAD *config.VADSectionDTO`(指针+omitempty,旧 bundle 缺段 nil 零回归;VAD 无密钥直接投影 DTO,与 PUT /api/config/vad 走同一 ApplyOverrides 路径)；**2026-08-15 P0 secrets key 白名单**:`importSecretsKeyAllowed`(白名单外的 key 如 HTTP_PROXY/PATH 拒绝导入,防经 secrets 机制注入进程环境)）
- `config_export_test.go` -- 配置导出/导入回归测试（29 个用例：密钥不泄漏含 MCP、可缺段含 MCP+VAD omitempty、持久化到 DB 含 MCP、非法值拒绝、env 改名 tombstone、回滚、overwrite 替换 secrets、旧备份兼容、**2026-07-23 新增 MCP round-trip + 同名碰撞双键 + 旧 bundle 零回归**、**2026-07-27 新增 VAD round-trip + VAD omitempty + 旧 bundle 零回归(VADExportSectionPresent/VADOmittable/ImportConfigVADRoundTrip/ImportConfigOldBundleLeavesVADUntouched)**、**2026-07-30 新增 Replay round-trip + Replay omitempty + 旧 bundle 零回归(ExportBundleReplaySectionPresent/ReplayOmittable/ImportConfigReplayRoundTrip/ImportConfigOldBundleLeavesReplayUntouched)**、**2026-08-14 新增 `TestExportConfigVADIncludesInaFields`(VAD ina_* 新字段进导出,PR#1 遗漏补丁)**、**2026-08-15 新增 secrets key 白名单 3 个(RejectsUnknownSecretKey 拒 HTTP_PROXY 等/AcceptsLegalMCPSecretKeys/AcceptsCustomEnvNameSecret)**）
- `server_test.go` -- 集成测试（92 个测试函数，表驱动子测试展开后运行时用例数更多；含 fake 依赖实现 + 测试辅助函数、stats/dashboard 死锁回归测试、2026-07-20 新增 `TestListBiliSeries_WithChannelID_*` 三个用 `cookieCapturingRT` 截获请求 Cookie + 覆盖 `server.biliCreativeClient` 让硬编码 B 站 URL 走桩、**2026-07-21 新增 `TestResetSession_*` 四个**：Success/NotFound/StatusNotFailed/NonASRFailure，验证 reset 端点的成功路径与 3 个 409 错误分支、**2026-07-25 `TestDiscoverPreviewByURL` 加 account_id body 解析 case**(preview-by-url 端点 body 字段 cookie_file→account_id;注意 newTestServer 的 discoveries Manager 未注入 cookieAccounts,account_id 传了也走空串降级,真实解密路径由 discover 包测试覆盖)、**2026-07-27 新增 `TestVADConfigRoundTrip`**(GET/PUT /api/config/vad 往返 + inline Validate 拒非法值)、**2026-07-30 新增 `TestReplayConfigRoundTrip`**(GET/PUT /api/config/replay 往返 + presence-aware + runtime_settings 持久化断言)、**2026-08-13 PR#1 +2**(`TestPublishConfigTypeErrorNamesField` JSON 类型错误字段级提示/`TestPublishConfigMalformedJSONKeepsGenericError` 非法 JSON 保持通用提示)、**2026-08-15 审核批次 +16**(H1 `/ws` 鉴权 4 个:RequiresToken/QueryToken/HeaderToken/NoTokenAnonymous;H2 onboarding 2 个:FreshDBNeeded/DismissThenNotNeeded;H7-notify test 端点 3 个:NotConfigured/NoopManager/SendError;M10 删账号 409 1 个;M12 copy-config 1 个;P0 MCP 脱敏 3 个:RedactedHeaderRoundTrip/RenameKeepsSecret/NewHeaderValueUpdates;L2 import 413 2 个:PayloadTooLarge/ChunkedOverLimit))。handler 包函数口径总数 126（server_test 92 + config_export_test 29 + auth_test 5）。
...
| 日期 | 操作 | 说明 |
|------|------|------|
| 2026-08-15/16 | BUG 修复/安全 | **审核批次 handler 六项**(commit `1c1898e`/`b728dcc`/`4175fa5`/`c514c16`/`122698d`+`df1b331`+`533a77a`/`6d33ccd`,P0×3 + M12 + H2 + H7 端点 + L2):① **H1 `/ws` WebSocket 补 token 鉴权**(query `?token=` / `X-Admin-Token` header 均可;配置了 admin_token 才强制,未配置保持匿名零回归;写超时防护防慢客户端卡住广播;前端 useWebSocket 同步带 query token);② **H2 onboarding dismissed 判断修正**(`secrets.Get` 不存在返回空串,被当「已 dismissed」→ needed 恒 false,新手引导条永不消失);③ **P0 MCP GET Authorization 脱敏**(回显 `__REDACTED__` 哨兵 + PUT 还原,配置页不再泄漏明文鉴权头);④ **P0 配置导入 secrets key 白名单**(`importSecretsKeyAllowed`,任意 key 如 HTTP_PROXY 不能再经 os.Setenv 注入进程环境);⑤ **M12 copy-config 源值污染 + 删账号 409 明细**(复制配置真正复制源主播的值而非被目标旧值反向污染;ErrAccountInUse 409 用 `strings.Cut` 取哨兵后缀携带引用主播数——`errors.Unwrap` 对 `%w (suffix)` 包装只能取回哨兵本身);⑥ **L2 importSession 2GiB 上限**(ContentLength 预检 + MaxBytesReader + `*http.MaxBytesError`→413)。测试 +16(server_test) + 3(config_export 白名单),handler 函数口径 107→**126**。 |
| 2026-08-13/14 | 功能/补丁 | **PR #1 handler 部分 + VAD 导出补丁**(commit `c9a3e51` + `ad7bda4`):① `jsonBindErrorMessage`——JSON body 类型不匹配时返回字段级提示(`invalid json type: expected number, got string`),非法 JSON 保持通用提示,前端能告诉用户哪个字段填错类型(投稿数字字段修复的后端半);② ad7bda4(维护者补丁,PR 审核 Medium 项):配置备份导出补齐 VAD 的 5 个 `ina_*` 新字段(PR#1 加字段时漏更新导出投影)。server_test +2、config_export_test +1,handler 函数口径 104→107。 |
| 2026-07-30 | 功能 | **回放类全局自动开关配置端点 + 配置导出/导入加 Replay 段 + 删除自定义时间段回顾端点**(branch `feat/replay-auto-switch-2026-07-30`)。**① 新端点 `GET/PUT /api/config/replay`**(与 VAD 段同模式,无密钥无 Validate):`replayConfigResponse`(auto_asr/auto_recap)+ `updateReplayConfig` 走 `ReplaySectionDTO` 基线拷贝 + ApplyOverrides。**② 配置导出/导入加 Replay 段**:`ConfigExportBundle.Replay *config.ReplaySectionDTO`(指针+omitempty,旧 bundle 缺段 nil 零回归);导入恢复走同一 ApplyOverrides 路径(基线拷贝保证旧 bundle 零回归)。**③ 删除自定义时间段回顾端点**:`POST /api/sessions/:sid/recap-partial` + `recap-with-range`(两者是同一功能别名,都调 `recap.CreateTaskWithRange`,无其他消费方);`generateRecapPartial`/`generateRecapWithRange` handler 删除。**测试**:server_test `TestReplayConfigRoundTrip`(+1) + config_export_test +4(ExportBundleReplaySectionPresent/ReplayOmittable/ImportConfigReplayRoundTrip/ImportConfigOldBundleLeavesReplayUntouched)。server_test 73→74、config_export_test 21→25、handler 函数口径 99→**104**。详见 `AGENTS.md` 2026-07-30 changelog + `plans/plan-replay-auto-switch-2026-07-30.md`。 |
| 2026-07-27 | 功能 | **ASR 前置 VAD 静音裁剪配置端点 + 配置导出/导入加 VAD 段**(commit `b1ba520`,branch `feat/vad-2026-07-27`)。**① 新端点 `GET/PUT /api/config/vad`**(与 tools 段同模式,runtimeconfig 持久化):`vadConfigResponse`(5 字段,DetectionMode 不暴露因 VADProcessor 固定 peak)+ `updateVADConfig` 走 `VADSectionDTO` 基线拷贝 + ApplyOverrides。**关键设计**:保存后**不**调 refreshRuntimeStatus(VAD 无 runtime 探测);`updateVADConfig` inline 调 `VADConfig.Validate()` 拒绝非法值(ThresholdDB 范围/MinSilenceSec>0 等),避免存入非法值后下次启动 `Config.Validate()` fatal(qoder v2 M-1)。**② 配置导出/导入加 VAD 段**:`ConfigExportBundle.VAD *config.VADSectionDTO`(指针+omitempty,旧 bundle 缺段为 nil 零回归);VAD 无密钥,直接投影 DTO(与 MCP 段的「密钥走 Secrets」不同,VAD 更简单);导入恢复走同一 ApplyOverrides 路径(与 PUT /api/config/vad 一致)。**测试**:server_test `TestVADConfigRoundTrip`(+1,GET/PUT 往返 + inline Validate 拒非法值) + config_export_test +4(VADExportSectionPresent/VADOmittable/ImportConfigVADRoundTrip/ImportConfigOldBundleLeavesVADUntouched)。server_test 72→73、config_export_test 17→21、handler 函数口径 94→**99**。详见 `AGENTS.md` 2026-07-29 changelog + `plans/plan-vad-2026-07-27.md` Phase 5/6。 |
| 2026-07-26 | 契约对齐 | **前后端 API 契约不一致对齐**(branch `fix/api-contract-2026-07-26`,qoder 计划审核 Ready with fixes + 代码审核 Ready)。**性质**:spec 与前端类型对齐型,**后端 Go 代码零改动**。涉及 handler 模块:① openapi.yaml 补 `POST /api/notify/test`(200 `{message}` + 409 未配置 + 500);② 补 `GET /api/bili/topics/search`(200 `{items:[{id,name,stat_desc}]}` + 502,前端 settings.ts:145 实际在调);③ `POST /api/sessions/import` 和 `DownloadByURLRequest` 的 `channel_id` 从 required 移除(后端空时回退 `_unassigned`,spec 比实现严)。详见 `AGENTS.md` 2026-07-26 changelog。handler 函数口径仍 94。 |
| 2026-07-25 | 功能 | **`POST /api/sessions/discover/preview-by-url` body 字段 `cookie_file`→`account_id`**(commit `860a581`,branch `fix/discover-account-select-2026-07-25`)。配合 discover 模块的 `resolveURLCookie` 改造(显式 account_id 分支复用 ResolveCookie 三级链)。`TestDiscoverPreviewByURL` 加 account_id body 解析 case(函数口径不变,仍是同一 Test 函数内新增子测试)。handler 函数口径仍 94。 |
| 2026-07-25 | BUG 修复 | **writeError 新增 `session.ErrAudioFileMissing`→409 映射**(branch `fix/media-ready-consistency-2026-07-25`,qoder 代码审核 Ready)。配合 session `ResetFailedSession` 守卫③.5(reset 前校验 audio.asr.mp3 真实存在),文件缺失时返回 409 Conflict(与同组 ErrLocalFilesRemoved/ErrActiveTaskExists 一致)。**未新增 handler 测试**(writeError case 是一行且与 4 个兄弟 case 同构,qoder Minor#3 评估 risk very low,接受)。handler 函数口径仍 94。见 `plans/plan-media-ready-consistency-2026-07-25.md` Phase 2。 |
