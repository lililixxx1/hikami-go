...
| GET | `/api/bili/series/list` | B 站文集列表（`listBiliSeries`,发布时合集选择;2026-07-06 起经 `biliCreativeGet` 共享 client + 风控对抗头） |
...
- **publish.private_pub 三态校验**(2026-07-08):PUT 校验接受 `0/1/2`(0=未设置/沿用 B 站默认,1=仅自己可见,2=可见范围)。允许 0 是 round-trip 保证:GET 在默认状态返回 0,PUT 必须能原样接回,否则干净默认配置保存报错 `private_pub must be 1 or 2`(bug 报告 #2 根因;其他 enum 字段 original/aigc/close_comment/up_choose_comment 本就接受 0,唯独 private_pub 之前不接受)。
...
- `server_test.go`: 集成测试（使用内存 SQLite 和 fake 依赖）。覆盖范围：
  ...
  - 任务 CRUD（创建、获取、取消、列表）
  - 配置导出/导入（merge/overwrite）—— 详见 `config_export_test.go` 的 11 个回归用例
...
- `config_export.go` -- 全量配置导出/导入处理器（ConfigExportBundle 6 段指针化 + WebDAVExportSection/ASRS3ExportSection 密钥剔除 DTO、handleExportConfig、handleImportConfig 两阶段事务化、validateImportedSections 持久化前校验、channelToUpsertInput）
- `config_export_test.go` -- 配置导出/导入回归测试（11 个用例：密钥不泄漏、可缺段、持久化到 DB、非法值拒绝、env 改名 tombstone、回滚、overwrite 替换 secrets、旧备份兼容）
- `server_test.go` -- 集成测试（50 个测试函数 + fake 依赖实现 + 测试辅助函数，含 stats/dashboard 死锁回归测试）
