...
- `server_test.go` -- 集成测试（67 个测试函数，表驱动子测试展开后运行时用例数更多；含 fake 依赖实现 + 测试辅助函数、stats/dashboard 死锁回归测试、2026-07-20 新增 `TestListBiliSeries_WithChannelID_*` 三个用 `cookieCapturingRT` 截获请求 Cookie + 覆盖 `server.biliCreativeClient` 让硬编码 B 站 URL 走桩）。handler 包函数口径总数 83（server_test 67 + config_export_test 11 + auth_test 5）。
