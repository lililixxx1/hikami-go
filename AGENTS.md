...
- `features/` — 按业务域组织(本次重构核心):
  ...
  - `features/recaps/components/`、`features/settings/components/`、`features/channel/`、`features/onboarding/` — 拆分后的子组件与自管理 hooks。`features/settings/components/` 下 9 张配置卡由 `SettingsView.vue` 编排为 4 个 `el-collapse` 折叠分组(总览/流水线配置/账号与备份/高级,`af9df47` 重构)。
