...
| 状态管理 | Pinia |
| 路由 | Vue Router 4 |
| UI 组件库 | V10 自建 H* 组件库(`src/components/ui/`,19 个 H* + HMessage/HConfirm/HToast；2026-07-15 新增 HCombobox) |
| HTTP 客户端 | Axios |
| Markdown 渲染 | marked |
| 类型系统 | TypeScript |
| 构建工具 | Vite |
...
│   ├── recaps/
│   │   ├── sessionActions.ts          # 场次操作纯函数（getRowActions/getDrawerActions/canFetchLocal/decideRetry/...；UIActionName 7 个状态推进型动作含 'reset' 2026-07-21 新增，edit/remove 已移除；isReplaySource 回放类隐藏 publish；isASRFailure 仅 ASR 失败的 failed 显示 reset）
│   │   ├── sessionActions.test.ts     # sessionActions 单测（静态 52 / 运行时 56 用例）
...
> **canvas 渲染规范（2026-07-18 确立）**：所有 `<canvas>` 元素**必须显式设置 HTML `width`/`height` 属性**（位图坐标系，与 CSS width/height 显示尺寸是两回事）。`<canvas>` 未设 HTML width/height 时浏览器内置默认 300×150，QRCode 等绘图库虽会内部重设但在某些时序下可能绘制异常（全黑/全白）。约定：模板里绑 `:width`/`:height` + JS 渲染函数里再显式设一次作双保险；渲染失败（如 canvas 未就绪）用 `console.warn` 让放弃可见，不静默 return。设置页 `AccountsCardV10.vue` 的 QR 区是 `<div v-if="qrSession">` 条件渲染，canvas 挂载晚于 `qrSession.url` 变化，renderQRCode 用 `nextTick + requestAnimationFrame`（+ 一帧重试）等 DOM 挂载后再画。
...
已配置 Vitest 测试框架。`vitest run` 运行时共 **242 个用例**，分布在 36 个测试文件中。Phase 6(V10 重写 + 移除 Element Plus)后新增 15 个 UI 组件单测(HButton/HCard/HCollapse/HDescriptions/HDialog/HDrawer/HEmpty/HInput/HPill/HProgress/HSelect/HSwitch/HTable/HTextarea/**HCombobox** 2026-07-15 新增),覆盖 H* 组件库核心交互。后续新增：features/home（RunningTasksSection + useElapsedDuration）、features/streamers（useStreamerDetail + **ChannelPublishConfig 12 用例 2026-07-20 新增**）、stores/sessions、features/recaps 组件测试（RecapDrawerV10/SessionTableV10）、features/settings（TemplateCardV10 extra_vars 编辑链路 8 用例）、**features/recaps/sessionActions +8 用例 2026-07-21 新增（failed 状态 reset 入口 + isASRFailure 识别，前端 192→200）**、**features/recaps/DiscoverPreviewDrawer 7 用例 2026-07-25 新增（发现回放抽屉首次有测试,Cookie 选择 HInput→HSelect 选账号,前端 200→207、27→28 文件）**、**features/settings/VADCardV10 5 用例 2026-07-27 新增（ASR 前置 VAD 静音裁剪配置卡片，加载/编辑/保存链路，前端 207→212、28→29 文件）**、**2026-08-13 PR#1 +5 文件/用例**（PublishCardV10 新测试文件 95 行覆盖数字字段 computed setter/SessionTableV10 +totalItems+activeTask 显示用例/DashScopeCardV10 临时存储开关/VADCardV10 ina 字段/MCPCardV10 头注释快照）、**2026-08-15/16 审核批次 +7 文件**（HInput type/list 透传 +13 行用例、useWebSocket 鉴权 89 行、useRecapDrawerContent 新文件 121 行、useSessionPagination 新文件 62 行、router/__tests__ 新文件 51 行、stores/tasks 71 行、DiscoverPreviewDrawer 勾选清空 +29 行），前端 212→**242**、29→**36** 文件。

关键测试文件:

- `features/recaps/sessionActions.test.ts`（`vitest run` 56 个用例，静态 52）：覆盖 `getRowActions`/`getDrawerActions`（各状态下可见动作；published 无状态推进型动作；回放类隐藏 publish；**failed 行 2026-07-21 新增 retry + reset 并存 + isASRFailure 守卫**）、`canFetchLocal`（local_available 守卫）、`decideRetry`/`isRetryable`/`retryHint`（重试决策）、`primaryActionType`（主按钮类型）、`UI_ACTION_REASON`（禁用原因，**UIActionName 2026-07-21 加 'reset' 共 7 个状态推进型动作**）、`isReplaySource`（download/import 判定）、**`isASRFailure`（current_task 类型=asr 的 failed 才显示 reset 入口）**。从 RecapsView 视图中抽出的纯函数。其中回放类行/抽屉动作用例包在 `describe.each(['download','import'])` 中，按两种 source_type 各跑一遍（运行时 4 个用例 ×2 = 8 个 describe.each 展开）。Phase 6 后 sessionActions 从 `@/api/types` 迁移到 `@/api/types-derived`。
...
| 日期 | 操作 | 说明 |
|------|------|------|
| 2026-08-15/16 | BUG 修复 | **审核批次前端六项**(commit `5cc7c00`(H5)/`b728dcc`(ws 前端)/`84c5e34`(L8-L12)/`3066149`(Minor 收尾),7H 批次):① **H5** HInput 透传 `type`/`list` props(password 密钥字段/number/datalist 不再落到 label 被浏览器忽略,`HInput.test.ts` +用例;DashScope API key 输入框同步改 `type="password"`);② **ws 前端**:`useWebSocket` 连接 URL 带 `?token=` query(配合后端 H1 /ws 鉴权,`useWebSocket.test.ts` 新文件 89 行覆盖鉴权头/query/未配置兼容);③ **L8** tasks store 照抄 sessions 范式加 inflight 去重(轮询与进度事件刷新并发只发一次 list,`stores/tasks.test.ts` +71 行);④ **L9** 新 `useSessionPagination` composable(列表收缩后 currentPage 收敛到新最后一页不渲染空表,新测试文件 62 行);⑤ **L10** `openRecap` 迁入新 `useRecapDrawerContent`(带 id 双重守卫,旧场次迟到响应不再覆盖新场次,新测试文件 121 行);⑥ **L11** DiscoverPreviewDrawer watch items 引用变化清空勾选(+29 行用例)+ **L12** `/tasks` redirect 映射 `session_id→?sid=` 保 query(新 `router/__tests__/index.test.ts` 51 行)。前端 212→**242** 测试、29→**36** 文件。 |
| 2026-08-13 | 功能 | **批量回放可靠性 PR #1 前端部分**(commit `c9a3e51`):① **翻页修复**:`SessionTableV10` totalPages 改用 `totalItems` prop(壳做过滤+分页后只传当前页 slice,旧代码用 `props.sessions.length` 算页数永远 1 页);② **回顾排队/生成中显示**:表格优先关联本场 pending/running 活跃任务(recap 成功前不推进 current_task_id,旧逻辑把排队中的回顾误显示成「转写完成」),状态列显示「回顾排队中/生成回顾中」;③ **投稿数字字段**:`PublishCardV10` private_pub/topic_id/list_id 三个 HSelect 加 computed setter 强制 `Number(v)`(HSelect 仅 emit string,直接 v-model 会把 JSON number 字段污染成 string,保存后后端 400;新 `PublishCardV10.test.ts`);④ DashScopeCardV10 加 `temporary_storage_enabled` 开关;⑤ VADCardV10 加 engine 选择 + 5 个 ina_* 字段;⑥ MCPCardV10 快照断言更新。前端 +5 文件(29→34 文件)+若干用例。 |
| 2026-07-27 | 功能 | **ASR 前置 VAD 静音裁剪配置卡片**(commit `b1ba520`,branch `feat/vad-2026-07-27`)。**改动**:① 新建 `features/settings/components-v10/VADCardV10.vue`(5 字段表单:开关/阈值 threshold_db/最短静音 min_silence_sec/padding/最低裁剪比 min_output_ratio;DetectionMode 不暴露因后端固定 peak);② `api/settings.ts` 新增 `getVADConfig`/`updateVADConfig` + `VADConfig`/`VADConfigUpdate` 类型(**过渡期手写**,spec 已补 `GET/PUT /api/config/vad` + `VADConfigResponse/Request` schema,但 `generated.ts` 尚未重生成,注释已标注待补后改走 `Schema<'VADConfigResponse'>` 派生);③ `SettingsView.vue` 设置页加 VAD 卡片折叠区;④ OpenAPI spec `docs/api/openapi.yaml` + `config-sections.yaml` 补 VAD 端点 + schema。**新建 `VADCardV10.test.ts` 5 用例**(加载渲染/字段编辑/emit save 载荷/disabled 态/保存后 toast),前端 207→**212** 测试、28→**29** 文件。type-check 0 error + vitest 全过。详见 `AGENTS.md` 2026-07-29 changelog + `plans/plan-vad-2026-07-27.md` Phase 7。 |
| 2026-07-26 | 契约对齐 | **前后端 API 契约不一致对齐**(branch `fix/api-contract-2026-07-26`,qoder 计划审核 Ready with fixes + 代码审核 Ready,后端零改动)。涉及前端:① `types-derived.ts` 删 RecapContent `bilibili` 幽灵字段 + ConfigStatus `glossary_configured/path` 幽灵字段 + source_type Omit 放宽补丁(失效死代码);② `settings.ts` `ToolsConfig`/`MCPConfig` 改 Schema<> 派生(MCPServerConfig/MCPBuiltinConfig 保留手写);③ `bili.ts` 三函数路径 `/api/bili/accounts*`→`/api/cookie-accounts`;④ settings.ts `tavily` 拼写修正(tavaly/tavali 错误变体);⑤ 3 个测试 fixture 配合收紧。type-check 0 error、vitest 28 文件 207 测试全过、零回归。详见 `AGENTS.md` 2026-07-26 changelog。 |
| 2026-07-25 | 功能 | **发现回放 Cookie 选择改为选已登录 B 站账号**(commit `860a581`,branch `fix/discover-account-select-2026-07-25`)。**改动**:① `DiscoverPreviewDrawer.vue` Cookie 输入 `HInput`→`HSelect`(移出 `<details>` 到主表单),复用 `ChannelPublishConfig` 的 `accountIdProxy`(string↔number\|null)+ `loadAccounts` 范式;下拉选项 = 「(默认) 使用默认登录账号」+ 已登录账号(`cookie_file !== ''` 过滤);② `api/sessions.ts` `previewDiscoverSessionsByURL(input)` body `cookie_file`→`account_id`;③ `api/generated.ts` 重生成(preview-by-url body schema)。**新建 `DiscoverPreviewDrawer.test.ts` 7 用例**(发现回放抽屉首次有测试:URL 必填校验/account 选项渲染/emit preview-submit/默认账号选项/未登录账号过滤/disabled 态/错误显示),前端 200→207 测试、27→28 文件。type-check 0 error + build 成功。 |
| 2026-07-21 | BUG 修复 | **failed 状态 reset 入口 + ASR 失败识别**(branch `fix/bug-fix-2026-07-20`,commit `61f3989` v6 + `add3b51` v7)。**触发**:ASR 任务失败后 session 进入 `failed` 状态,重提 ASR 返回 `409 status must be media_ready`,**前端无任何恢复入口**——用户只能改 DB + 重启服务。**改动**(4 文件):① `api/sessions.ts` 新增 `resetSession(sid)` 调 `POST /api/sessions/:sid/reset`;② `features/recaps/sessionActions.ts` 加 `UIActionName 'reset'`(共 7 个状态推进型动作)+ `isASRFailure(session)` helper(仅 current_task 类型=asr 的 failed 才显示 reset 入口)+ `getRowActions` 的 failed 分支(retry 与 reset 并存);③ `features/recaps/components/SessionTableV10.vue` 加 emit + 按钮;④ `views/RecapsView.vue` 加 `handleReset` + HConfirm 确认对话框。**测试**:sessionActions +8 用例(覆盖 failed 行 retry/reset 并存、非 ASR 失败无 reset、isASRFailure 识别、回放类 failed 仍无 reset),前端 192→200 测试、27 文件不变。type-check + build 通过。 |
| 2026-07-20 | 功能 | **主播级发布配置 UI 补齐 + ChannelPublishConfig 新组件**(branch `fix/streamer-publish-fields-2026-07-19`,codex 4 轮计划审核 r17/r17b/r17c/r17d 收敛)。**背景**:`StreamerDrawer.vue` 的「发布设置」section 此前只有「自定义封面」,其余 6 个 per-channel 发布字段(publish_account_id/list_id/mode/private_pub/original/aigc)后端已就绪但前端无 UI;`listBiliSeries` 端点也无 channel 维度(只走全局默认账号,无法支持不同主播不同发布账号)。**改动**:① **新建 `features/streamers/components/ChannelPublishConfig.vue`**:7 字段表单(账号/文集/模式/可见范围/原创/AIGC/封面) + 「保存发布设置」按钮(对比 draft vs channel 仅 emit 变化字段);**关键 codex r17b BLOCKING**:HSelect 仅 emit string,数字字段用 computed 代理 `Number(v)`,账号字段特殊(空串 → null,否则 `Number(v)`,因为「跟随全局」是 null 而非 0)。② **`useStreamerDetail.ts`**:RecapOverrideField 去掉 publish_cover_url(改归 PublishOverrideField);新增 `handlePublishOverrides(changes)` 单次 updateChannel 调用(必须 `{ ...toInput(c), ...changes }` 完整基底,因为 updateChannel 全量 PUT);删除 `saveCover`(并入新方法);toInput 补 publish_account_id。③ **`StreamerDrawer.vue`**:删旧 cover 表单,引入 ChannelPublishConfig,emit `save-publish` 替代 `save-cover`。④ **`ChannelAdvancedConfig.vue`**:补 6 字段只读展示,**各字段「跟随全局」哨兵不同**(list_id 仅 -1,private_pub 仅 0,original/aigc 仅 -1),逐字段独立函数避免误导。⑤ **`StreamersView.vue`** 壳:删 `onSaveCover`,加 `onSavePublish`。⑥ **`api/settings.ts`**:`listBiliSeries(channelId?)` 加可选参数。**测试**:`ChannelPublishConfig.test.ts` 12 用例(6 字段渲染/draft 同步/account 加载/series 按 channel_id 懒加载/错误显示/单字段 emit/无变化禁用/混合保存/HSelect string→number 转换/账号空串→null/updating 禁用),前端 180→192 测试、26→27 文件。type-check 0 error、build 通过。 |
| 2026-07-19 | 功能 | **主播管理 ↔ 回顾管理·回放 完全解耦**(branch `fix/decouple-recap-replay-2026-07-18`,codex 计划 v1/v2 审核 + 代码审核)。三处前端改动:① **DiscoverPreviewDrawer 重写**:顶部新增 URL 输入区(`HInput` URL 必填 + `<details>` 高级折叠含 cookie 路径 + title_prefix + 「发现」按钮),触发 `emit('preview-submit', input)`;原「打开即自动遍历主播表」的旧行为删除,用户不输入 URL 不发起预览。原「全部下载」按钮删除(URL 模式下用「全选新回放」+「执行勾选」替代)。`_unassigned` channel 显示「未分类」标签。② **`DownloadByURLDrawer.vue` + `ImportSessionDrawer.vue`**:主播下拉第一项加 `{label:'(不选) 归入未分类', value:''}`,校验从「channel_id 必填」改为「channel_id 可选」;label 加 `(可选)` 灰字。ImportDrawer 的「该主播自动 ASR」在 selectedChannel=null 时显示「未选主播,使用全局 ASR 配置」。③ **`SessionFiltersV10.vue`**:加 `activeTab` prop,replay tab 隐藏主播筛选下拉(回放类不再绑真实主播)。④ **`SessionTableV10.vue`**:`channelName()` helper 把 `_unassigned` 显示为「未分类」。⑤ **`api/sessions.ts`**:新增 `previewDiscoverSessionsByURL(input)`;`RecapsView.openDiscover()` 不再调预览,新增 `handleDiscoverSubmit(input)`。无新增 vitest(纯 UI/类型,chrome-devtools-mcp 实测)。type-check + 180 测试 + build 全过。 |
| 2026-07-18 | 修复 | **B 站扫码二维码渲染修复 + canvas 渲染规范确立**（branch `fix/investigations-2026-07-18`，codex 计划审核 APPROVED）。基于 `docs/扫码二维码问题分析.md`（调查文档误标"已修复"，实际未落地）。3 个根因：① **设置页首次点击必现空白**（`AccountsCardV10.vue` 的 `watch(qrSession.url)` 在 `<div v-if="qrSession">` 内 canvas 挂载前同步触发，`!canvasRef.value` → return → 永不画）；② **canvas HTML width/height 属性未设**（默认 300×150，防御性加固，codex Medium 提示因果链未从库实现完全证明）；③ **「刷新状态」按钮被拉成 1505px 长条**（flex 容器缺 `align-items: flex-start`）。修复：`AccountsCardV10.vue` 抽 `renderQRCode`（`nextTick + requestAnimationFrame` 双等 + 一帧重试 + 失败 `console.warn`）、canvas 加 `:width="200" :height="200"` + CSS 尺寸、flex 容器加 `align-items: flex-start`；`BiliQRCodeLoginDialog.vue` canvas 加 `:width="220" :height="220"` + renderQRCode 显式设位图尺寸。**保留 07-15 的 `{ immediate: true }` 不改回 onMounted**（调查文档建议但本计划不采纳，避免无收益回归面）。无新增测试（竞态/时序 + CSS 视觉类，靠 chrome-devtools-mcp 像素级实测）。vitest 26 文件 180 测试全过、type-check 0 error、build 通过。新增「canvas 渲染规范」（见 §BiliQRCodeLoginDialog 模块详解）：所有 `<canvas>` 必须显式设 HTML width/height 属性。 |
| 2026-07-16 | 修复 | **TemplateCardV10「添加变量」无效 + 配套健壮性加固**（调查文档修复批次，codex 3 轮 + code-reviewer APPROVED）。① **kvRows 解耦**：`TemplateCardV10.vue` 的 `kvRows` 从 writable computed 改为独立 `ref<KVRow[]>`（每行带稳定 id），消除读写环（getter 依赖 extraVars、setter 写 extraVars 且 if(k) 丢弃空 key → 点「+ 添加变量」追加空行被 setter 写回 '{}' → getter 重算销毁新行）。新增 `syncKvRowsFromExtraVars`（loadData/导入后从 extraVars 填充）和 `flushKvRowsToExtraVars`（仅在 handleSave 前调用，丢弃空 key）。`:key` 从数组索引改 `row.id`。② **成功/失败协议**（codex BLOCKING）：`useRecapTemplateEditor` 的 `loadData`/`save`/`importTemplateFile` 改返回 `Promise<boolean>`，组件仅在成功时 sync/关对话框/emit('saved')；save/import 内部还检查 `loadData()` 返回值（写入成功但重拉失败也视为失败）。③ **导入同步**（codex B1）：import handler 后加 `syncKvRowsFromExtraVars`，避免导入后 kvRows 保持旧行、下次保存静默覆盖导入数据。④ **预设不 sync**（codex BLOCKING）：`handleApplyPreset` 不再调 sync（applyPreset 只改 system_prompt/user_format，不动 extraVars，sync 会丢未保存编辑）。新增 `TemplateCardV10.test.ts`（8 用例：添加行不消失/空key丢弃/删除不串行/真实导入同步/预设取消不丢失/导入失败保留态/保存失败不emit/导入成功但重载失败）。前端 161→180 测试、24→26 文件，type-check + build 通过。 |
| 2026-07-15 | 功能 | **回顾模型支持手动输入 + HCombobox 组件**（`e17fa9c`，codex 审核 2 轮 APPROVED）。回顾模型选择原先用 `HSelect`（原生 `<select>`），只能从预设里选、无法手动输入；后端预设也多达 8 个。改为：① 新增 `components/ui/HCombobox.vue`（input + 原生 datalist 组合框）：可输入任意模型名 + 下拉快捷选项，`clearable` 提供清空回路（主播级"留空跟随全局"），渐进增强（旧浏览器/读屏不支持 datalist 时降级为普通 input）。H* 组件库 16→17（后续文档统计口径含 ConfirmHost/HToast 为 19）。② `RecapCardV10.vue`（设置页）与 `StreamerDrawer.vue`（主播抽屉）两处回顾模型字段从 `HSelect` 改用 `HCombobox`。③ 后端 `recommendedRecapModels` 精简到 DeepSeek 2 个（flash + pro）。新增 `HCombobox.test.ts`（11 用例：v-model/旧值回显/datalist 绑定/同页双实例 id 唯一性/disabled/clearable/slot/size）。前端测试 +11。 |
| 2026-07-15 | 修复 | **4 个调查问题修复批次（UI 部分）**（`a1a595d`，codex 计划+执行审核 APPROVED）：① **RecapDrawerV10 面板 z-index 缺失**：面板用 `recap-drawer-panel` class 但该 class 全前端无 CSS 定义 → `position:static`/`z-index:auto` → 被遮罩（z-index:100）盖住、回顾内容不可见。面板 class 改为 `drawer rtl open recap-drawer-panel`（复用 `ui.css` 的 `position:fixed` + `z-index:101` + 右侧滑出 + 归位）。② **B站扫码二维码首次不显示**（`BiliQRCodeLoginDialog.vue` + `useBiliQRCodeLogin.ts`）：真实根因（修正调查文档的竞态归因）是 `watch(visible)` 缺 `immediate:true`——组件用 `v-if` 挂载、`visible` 初值即 true，`watch` 默认只在值变化时触发 → `startLogin` 永不调用。主修复：`watch` 加 `{ immediate: true }`；补充防御：`renderQRCode` 加 `await nextTick()` 确保 canvas 挂载就绪。无新增测试（竞态/时序类，靠手动 + codex 审核）。前端 180 测试不变。 |
